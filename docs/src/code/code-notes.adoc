...
* 'jogmod' - one-dimensional planning for jogging (joint or axis)
or external offset motion.

* 'tpmod' - multi-dimensional trajectory planning in Cartesian
coordinates
...
realtime motion control, the script first loads the default jogmod,
tpmod, and homemod modules and then loads the kinematics and motion
modules according to settings in halfiles specified by the ini
file.

Custom (user-built) jogging, homing, or trajectory-planning modules
can be used in place of the default modules via ini file settings
